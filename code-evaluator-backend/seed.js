...
A subsequence of a string is a new string generated from the original string with some characters deleted without changing the relative order.
...
async function seed() {
  try {
    ...
    if (existingCount === 0) {
      ...
      console.log(`ℹ️  Problems already exist (${existingCount} found). Skipping problem seed.`);
    }

    // Promote user to admin
    const adminEmail = "shashvatt68@gmail.com";
    ...
    if (user) {
      ...
      console.log(`⚠️  User ${adminEmail} not found — make sure they have registered first`);
    }
